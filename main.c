...
volatile uint16_t raw5 = 0, raw20 = 0, raw21 = 0;
volatile float v5 = 0.0f, v20 = 0.0f, v21 = 0.0f;
volatile float t5 = 0.0f, t20 = 0.0f, t21 = 0.0f;
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc) {
	memcpy(rawTempReadings + MUX_BANK_COUNT*selectedCell, tempBuffer, MUX_BANK_COUNT);
...
void writeTemperaturesOverCan(uint8_t cellBank) {
	CAN_TxHeaderTypeDef txHeader = {
		.StdId = 0x00, // Need to calculate ID for data
		.ExtId = 0x00,
		.IDE = CAN_ID_STD,
		.RTR = CAN_RTR_DATA,
		.DLC = 8
...
int main(void)
{
  ...
  selectedCell = 0;
  memset(tempBuffer, 0, sizeof(*tempBuffer)*MUX_BANK_COUNT);
  memset(rawTempReadings, 0, sizeof(*rawTempReadings)*MUX_BANK_COUNT*CELLS_PER_MUX);
  /* USER CODE END Init */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  if (dataReady) {
		  ...
		  // Calculate temps here
		  // Trigger fault
		  // Send rawTempReadings over can
	  }
  }
  /* USER CODE END 3 */
}
